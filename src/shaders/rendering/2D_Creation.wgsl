...
fn vs_main(
    ...
    let id = instance + c_input.starting_index;
    $ 2D {
        let aspect = input.width / input.height;
        let xy = 2.0 * scale * vec2(in.position.x / aspect, in.position.y);
        let center = scale * vec2(pos[id].x / aspect, pos[id].y);
        let off = vec2((input.xOff + input.ui_xOff + c_input.mouse_x) / aspect, (input.yOff + input.ui_yOff + c_input.mouse_y)) * (scale);
        out.clip_position = vec4(xy * radii[id] + center + off, 0.0, 1.0);
    }
    $ 3D {
        let cam_pos = input.cam.pos.xyz;
        let part_pos = vec3(pos[id].x, pos[id].y, 0.0);
        ...
        let center = input.ui_scale * vec3(c_input.mouse_x, c_input.mouse_y, 0.0);
        let off = vec3(input.ui_xOff, input.ui_yOff, 0.0) * input.ui_scale;

        let final_pos = xy * radii[id] + center + off;
    ...
    out.rot = 0.0;//rot[id];
    out.rot_vel = 0.0;//  rot_vel[id];
    out.id = id;
    out.selected = selections[id];
    out.w_h = vec2(i32(input.width), i32(input.height));
    out.pixel = out.clip_position.xy;
    out.vel = vec2(0.0);//vel[id];
    out.scale = scale;

    if settings.colors == 0 {
        out.color = vec3(0.05, 0.05, 0.05);
    } else if settings.colors == 1 && material_pointers[id] != -1 {
        out.color = vec3(
            srgb_to_linear(materials[(material_pointers[id])].red),
            srgb_to_linear(materials[(material_pointers[id])].green),
            srgb_to_linear(materials[(material_pointers[id])].blue)
        );
    } else if settings.colors == 2 {
        let seed1 = u32(rand(instance + c_input.particles, 4294967296.0));
    ...
    $ COLOR-ROTATION {  
        ...
        if settings.color_code_rot == 1 {
            if in.rot_vel > 0.0 {
                ...
                color = vec4(q, color.r, 0.0, color.ba);
            }
        }
    }
