...
impl State {
    pub fn new(config: &mut WGPUConfig, settings: &mut Settings, script_manager: &ScriptManager) -> Self {
        ...
        let mut contacts = vec![bytemuck::cast::<i32, f32>(-1); CONTACT_SIZE * settings.setup.max_contacts * p_count * PREALLOC];
    ...
    pub fn print_state(&self) {
        for i in 0..1 {
            ...
            for j in 0..CONTACT_SIZE {
                ...
                // if b != -1 {
                println!("{}\n", self.contacts[base + j]);
                // }
                //     println!("    {}:", j);
                //     println!("b: {}, ", b);
                //     base += 1;
                //     println!("forces: ({}, {}), ", self.contacts[base + 1], self.contacts[base + 2]);
                //     println!("moment: {}, ", self.contacts[base + 3]);
                //     base += 1;
                //     println!("s_force: ({}, {}), ", self.contacts[base + 4], self.contacts[base + 5]);
                //     println!("theta_b: {}, ", self.contacts[base + 6]);
                //     println!("bond_type: {}, ", bytemuck::cast::<f32, i32>(self.contacts[base + 7]));
                //     base += 1;
                //     println!("bond_length: {}, ", self.contacts[base + 8]);
                //     println!("bond_angle: {}, ", self.contacts[base + 9]);
                //     print!("\n");
                // }
            }
        }
            ...
            for j in i..self.p_count {
                if i != j {
                    ...
                    if distance < sum_of_radii * 1.05 {
                        // bond detected
                        if col_num < MAX_BONDS {
                            //&& bonds[(i * MAX_BONDS + col_num) * 3] == -1 {
                            // CREATE BOND
                            ...
                            for k in settings.setup.max_contacts * i..settings.setup.max_contacts * (i + 1) {
                                ...
                                if bytemuck::cast::<f32, i32>(contacts[CONTACT_SIZE * k]) == -1 {
                                    ...
                                    contacts[CONTACT_SIZE * k + 6] = 0.0; // shear force
                                    contacts[CONTACT_SIZE * k + 7] = 0.0; // bond shear force
